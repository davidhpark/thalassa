...
def softmax_fw_ref(x: torch.Tensor) -> torch.Tensor:
    # x: [M, N]
...
def softmax_fw_triton(x: torch.Tensor, BLOCK_N=1024) -> torch.Tensor:
    M, N = x.shape
    y = torch.empty_like(x)
    grid = lambda meta: (M,)
    softmax_fw_kernel[grid](
        x, y,
        M, N,
...
def bench_fw(fn, x, iter=200, warmup=50):
